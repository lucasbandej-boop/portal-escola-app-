  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: '#f0f2f5' }}>
      ...
      <View style={styles.fbInfoCard}>
        ...
        <Text style={styles.fbInfoRow}>📞 Contacto / NIF: {dados.nif}</Text>
        {tipo === 'escola' ? (
          <>
            <Text style={styles.fbInfoRow}>👨‍💼 Director: {dados.director || 'N/A'}</Text>
            <Text style={styles.fbInfoRow}>👨‍💼 Vice-Director: {dados.vice_director || 'N/A'}</Text>
            <Text style={styles.fbInfoRow}>👨‍🏫 Nº Professores: {dados.num_professores || '0'}</Text>
            <Text style={styles.fbInfoRow}>👨‍🎓 Nº Estudantes: {dados.num_estudantes || '0'}</Text>
            <Text style={styles.fbInfoRow}>📍 Localização: {dados.localizacao || 'N/A'}</Text>
            <Text style={styles.fbInfoRow}>📚 Classes: {dados.classes || 'N/A'}</Text>
          </>
        ) : null}
      </View>
...
function FormCadastramentoInstituicao({ usuario, onConcluir, onCancelar }) {
  ...
  const submeter = async () => {
    ...
    try {
      ...
      Alert.alert('Sucesso 🎉', 'Instituição salva na base de dados com sucesso!');
      onConcluir(payload);
    } catch (err) {
      Alert.alert('Erro no Servidor', err.message || 'Ocorreu uma falha ao tentar conectar.');
    } finally {
      setLoading(false);
    }
  };
